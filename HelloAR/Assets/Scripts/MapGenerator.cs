...
public class MapGenerator : MonoBehaviour
{
	...
	public int roomThresholdSize = 40;      //清除小孔的的阈值。
	...
	private void GenerateMap()
	{
		...
		DrawCircle (new Coord(width/2, height/2), passageWidth);
	
		Coord newCoord;
		int ran = (new System.Random ()).Next(1, 5);
		print (ran);
		if (ran == 1)
			newCoord = new Coord (6, 6);
		else if (ran == 2)
			newCoord = new Coord (6, height - 6);
		else if (ran == 3)
			newCoord = new Coord (width - 6, height - 6);
		else
			newCoord = new Coord (width - 6, 6);
		DrawCircle (newCoord, passageWidth);
		print (newCoord.tileX + " " + newCoord.tileY);
		Vector3 newPosition = CoordToWorldPoint (newCoord);
		newPosition.y = 5.0f;
		anotherBall.transform.position = newPosition;
		//anotherBall.transform.position.y = 5.0f;
